...
- [x] アクセシビリティ確認（コードレベルでのレビューと修正: 認証後の画面に見た目には出さない
      ...
      `aria-label` を付与、フォーカスリングを消す CSS がないことを確認。axe や Lighthouse
      等の自動監査・実際のスクリーンリーダーでの確認はこのセッションでは未実施）
